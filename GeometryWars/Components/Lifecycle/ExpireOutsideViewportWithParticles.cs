...
public sealed class ExpireOutsideViewportWithParticles : Component
{
    ...
    public ExpireOutsideViewportWithParticles(IParticleSystem<ParticleState> particles, FrameInfo frame, Texture2D lineParticle)
    {
    ...
    public override void PostUpdate(Entity owner)
    {
        ...
        for (int i = 0; i < GameSettings.Visuals.BulletDeathParticles; i++)
        {
            _particles.CreateParticle(_lineParticle, _transform.Position, Color.LightBlue, 50, 1,
                ParticleState.JitteredTrail(Random.Shared.NextVector2(0, 9)));
        }
    }
}
